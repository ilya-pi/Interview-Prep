...
func insertNodeAtPosition(llist *SinglyLinkedListNode, data int32, position int32) *SinglyLinkedListNode {
	...
	// prepend list
	if position == 0 {
		n := &SinglyLinkedListNode{data: data, next: llist}
		return n
	}
	// insert at position
	cur := llist
	// we allow overflow
	// we already stepped into the head
	for ; position > 1 && llist != nil; position-- {
		cur = cur.next
	}
	prevNext := cur.next
	cur.next = &SinglyLinkedListNode{data: data, next: prevNext}
	return llist
	...
	// // Edge case for head
	// for k := int32(0); k < position-1 && pos != nil; pos, k = pos.next, k+1 {}
	// n := &SinglyLinkedListNode{ data: data, next: pos.next }
	// pos.next = n
	// return llist
}
