...
func sortedInsert(llist *DoublyLinkedListNode, data int32) *DoublyLinkedListNode {
	if llist == nil {
		n := &DoublyLinkedListNode{data: data, next: nil, prev: nil}
		return n
	}
	/*
	   2 -> 3 -> nil
	*/
	fbn := llist
	prev := fbn.prev
	for ; fbn != nil && fbn.data < data; fbn, prev = fbn.next, fbn {
	}
	if prev == nil {
		n := &DoublyLinkedListNode{data: data, next: fbn, prev: nil}
		return n
	}
	if fbn == nil {
		n := &DoublyLinkedListNode{data: data, prev: prev, next: nil}
		prev.next = n
		return llist
	}
	n := &DoublyLinkedListNode{data: data, prev: fbn.prev, next: fbn}
	fbn.prev.next = n
	fbn.prev = n
	return llist
}
